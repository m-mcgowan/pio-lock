...
class TestScanEnvLibraries:
    ...
    def test_registry_library(self, tmp_project, make_registry_lib, mock_git_commands):
        ...
        libs = pio_lock.scan_env_libraries(tmp_project, "testenv")
        assert len(libs) == 1
        assert libs[0] == {
            "name": "MyLib",
            "type": "registry",
    ...
    def test_registry_library_without_owner(
        ...
        # Rewrite .piopm without an owner
    ...
    def test_git_library_with_library_json(self, tmp_project, make_git_lib, mock_git_commands):
        make_git_lib(
            "testenv",
            "ugly-dirname",
            sha="aaa111",
    ...
    def test_missing_libdeps_dir(self, tmp_project, mock_git_commands, capsys):
        libs = pio_lock.scan_env_libraries(tmp_project, "noenv")
        assert libs == []
...
class TestScanGlobalPackages:
    """Tests for scan_global_packages."""
    ...
    def test_captures_matching_prefixes(self, make_global_packages):
        ...
        pkgs = pio_lock.scan_global_packages()
