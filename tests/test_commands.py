"""Tests for the capture, restore, and check commands."""
...
import pio_lock
from tests.conftest import write_lockfile
...
class TestCapture:
    ...
    def test_capture_creates_lockfile(
        ...
        rc = pio_lock.capture(tmp_project, ["testenv"])
        assert rc == 0

        lockfile = tmp_project / "pio.lock.json"
    ...
    def test_capture_custom_output(
        ...
        rc = pio_lock.capture(tmp_project, ["testenv"], output_path=output)
        assert rc == 0
    ...
    def test_capture_multiple_envs(
        ...
        rc = pio_lock.capture(tmp_project, ["env1", "env2"])
        assert rc == 0
    ...
    def test_capture_fails_without_platformio_ini(self, tmp_path, mock_git_commands):
        rc = pio_lock.capture(tmp_path, ["testenv"])
        assert rc == 1
...
class TestCheck:
    ...
    def test_check_passes_when_matching(
        ...
        rc = pio_lock.check(tmp_project, ["testenv"])
        assert rc == 0
    ...
    def test_check_detects_missing_library(
        ...
        # Lockfile says a lib should exist, but libdeps dir is empty
...
class TestRestore:
    ...
    def test_restore_skips_already_installed(
        ...
        assert rc == 0
        # pio pkg install should NOT have been called
...
class TestLibInstallSpec:
    ...
    def test_registry_with_owner(self):
        spec = pio_lock._lib_install_spec(
            {
                "type": "registry",
                "name": "Foo",
                "version": "1.2.3",
    ...
    def test_local_returns_none(self):
        assert pio_lock._lib_install_spec({"type": "local"}) is None

    def test_unknown_returns_none(self):
        assert pio_lock._lib_install_spec({"type": "magic"}) is None
...
class TestCLI:
    ...
    def test_capture_args(
        self, tmp_project, make_registry_lib, mock_git_commands, make_global_packages
    ):
        make_registry_lib("testenv", "Lib", project_dir=tmp_project)
        make_global_packages("toolchain-x", "1.0")
        ...
        assert rc == 0
        assert (tmp_project / "pio.lock.json").exists()

    def test_check_args(self, tmp_project, make_registry_lib, mock_git_commands):
        make_registry_lib("testenv", "Lib", "1.0.0", project_dir=tmp_project)
        write_lockfile(
            tmp_project,
            {
                "testenv": [
                    {"name": "Lib", "type": "registry", "version": "1.0.0", "owner": "testowner"},
                ],
            },
        )
        rc = pio_lock.main(
    ...
    def test_restore_args(self, tmp_project, make_registry_lib, mock_git_commands):
        make_registry_lib("testenv", "Lib", "1.0.0", project_dir=tmp_project)
        write_lockfile(
            tmp_project,
            {
